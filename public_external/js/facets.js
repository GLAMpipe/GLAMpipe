function refjyx (config) {
	...
	this.checkSelection = function (facet, item, item_count) {
		var value = encodeURIComponent(item._id)
		var facet_value = value.replace(/'/g, '%27'); // encode single quote
		if(facet.selections.indexOf(facet_value) !== -1)
			return "<input type='checkbox' checked id='"+facet.key + item_count +"' data-value='"+facet_value+"'/>";
		else 
			return "<input type='checkbox' id='"+facet.key + item_count +"' data-value='"+facet_value+"'/>";
