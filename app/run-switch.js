				...
				case "web":
					...
					switch (node.subsubtype) {
						...
						case "csv":
							...
							// empty previous imports if mode is "import"
							web.downloadAndSave (node, download, false, function() {
								if(node.settings.mode === "import") {
									var query = {}; 
			...
			switch (node.subtype) {
						...
						case "script":
							try {
								...
								asyncLoop.documentLoop(node, sandbox, function ondoc (doc, sandbox, next) {
									sandbox.run.runInContext(sandbox,{timeout:100});
									next();
								});
							} catch (e) {
								console.log("ERROR:", e);
				...
				case "lookups":
					...
					switch (node.subsubtype) {
						...
						case "mongo":
							var mongo = require("../app/node_runners/mongo.js");
							asyncLoop.fieldLoop(node, sandbox, mongo.query);
