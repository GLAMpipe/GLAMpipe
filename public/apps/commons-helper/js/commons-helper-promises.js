...
function Helper() {
	...
	self.checkIfCommons = function(button) {
		...
		return self.gp.runNodeSingle(config.nodes.download, doc)
			...
			.then(function(data) {
				if(data.result.value.includes("AAAA_error")) {
					button.replaceWith("Can't read the file!");
			...
			.then(function(data) {
				if(data.result.value == "") {
					...
					button.replaceWith("<a target='_blank' href='" + data.result.value + "'>File is already in Commons!</a>");
					throw("File is already in Commons!")
				}
			})
			.catch(function(status) {
	...
	self.checkIfCommons_old = function(button) {
		var doc = button.data("id");
		return self.gp.runNodeSingle(config.nodes.download, doc)
			.then(function(data) {
				return self.gp.runNodeSingle(config.nodes.checksum, doc)
			...
			.then(function(data) {
				if(data.result.value == "") {
					...
					button.replaceWith("<a href='" + data.result.value + "'>file exists!</a>");
					throw("File is already in Commons!")
				}
			})
			.then(function(data) {
	...
	self.renderData = function(cb) {
		...
		$.getJSON(url, function(docs) {
			docs.data.forEach(function(doc, index) {
				...
				html += "<td><a target='_blank' href='" + doc["path"] + "'>link</a></td>"
				html += "<td><button type='button' class='btn btn-primary check' data-id='"+doc["_id"]+"'>Check!</button></td></tr>";
